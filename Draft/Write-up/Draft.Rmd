...
where $\bar{d_i,_j} = m^{-1}\sum_{t=1}^{m}{d_i,_j,_t }$ measures the sample average loss differential between model $i$ and $j$ and  $\hat{var[\bar{d_i,_j}]}$ is estimated through bootstrapping as described in @hansen2011. 

This paper uses a block–bootstrap procedure of 5000 resamples as in @hansen2011. The block length $p$ is chosen by fitting an AR(p) model to the $d_i,_j,_t$ terms and determining the maximum number of significant AR parameters obtained. While $t_i,_j$ has a t-distribution, $T_R$ has a non-standard distribution under the null hypothesis hence is estimated by implementing a bootstrapping procedure similar to that used to estimate $\hat{var[\bar{d_i,_j}]}$. 
